import React from 'react';
import { Select } from 'antd';

type Props = {
	options: { value: string; label: string }[];
	labelName?: string;
	onChange: (value: { value: string; label: string }) => void;
	isError?: boolean;
	errorText?: string;
	className?: string;
	wrapperClassName?: string;
	initialValue?: { value: string; label: string };
};

const SelectWithLabel = ({ options, labelName, isError, errorText, onChange, className, wrapperClassName, initialValue }: Props) => {
	return (
		<div className={`flex flex-col gap-1 ${wrapperClassName}`}>
			{!!labelName && <label className='text-xs my-0 py-0'>{labelName}</label>}
			<Select options={options} onChange={onChange} className={className} size='large' defaultValue={initialValue} />
			{isError && <p className='text-[10px] py-2 text-red-400'>{errorText}</p>}
		</div>
	);
};

export default SelectWithLabel;
